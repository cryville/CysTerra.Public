...
namespace Cryville.EEW.FANStudio.TTS {
	public class SichuanEEWTTSMessageGenerator : IContextedGenerator<SichuanEEW, ITTSMessageGeneratorContext, TTSEntry?> {
		readonly ReportUnitStateList _states = new();
		public TTSEntry? Generate(SichuanEEW e, ITTSMessageGeneratorContext? context, ref CultureInfo culture) {
			...
			sb.Append(string.Format(culture, res.GetStringRequired("Revision"), e.Updates));

			var pr = _states.Push(id, [(int)(e.EpicenterIntensity is float epiIntensity ? ((epiIntensity - 2) / 1.2f) : (e.Magnitude - 2))]);
