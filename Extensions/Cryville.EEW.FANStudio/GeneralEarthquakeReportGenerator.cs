...
namespace Cryville.EEW.FANStudio {
	public abstract class GeneralEarthquakeReportGenerator {
		...
		public ReportModel Generate(GeneralEarthquake e, IReportGeneratorContext? context, ref CultureInfo culture) {
			...
			if (e.Magnitude is float mag) result.Properties.Add(new(TagTypeKeys.Magnitude, res.GetStringRequired("PropertyMagnitude"), mag.ToString("F1", culture), context.SeverityScheme, mag) { AccuracyOrder = 10 });
			if (e.Depth is float depth) result.Properties.Add(new(TagTypeKeys.HypocenterDepth, res.GetStringRequired("PropertyDepth"), string.Format(culture, res.GetStringRequired("PropertyDepthValue"), depth.ToString("F0", culture)), context.SeverityScheme, depth) { AccuracyOrder = 10 });
			return result;
		}
